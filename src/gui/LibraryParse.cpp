...
{
  do
  { // Outer loop to parse all libraries
    // need to throw exception
    if (mParse->readTag() != "library")
      return;
    mParse->readTag();
      ...
      do
      {
        ...
        if ((temp == "input") || (temp == "output"))
        {
          ...
          do
          {
            ...
            {
              if (hsType == "output")
              { // Only outputs can have <enable_input> tags.
        ...
        else if (temp == "caption")
        {
          ...
          if (newGate.caption == "Inverter" && (time(0) % 1001 == 0))
          { // Easter egg, rename inverters once in a while :)
...
bool LibraryParse::getGate(string gateName, LibraryGate &lgGate)
{
  ...
  if (findGate == wxGetApp().gateNameToLibrary.end())
    return false;
  map<string, LibraryGate>::iterator findVal = gates[findGate->second].find(gateName);
  if (findVal != gates[findGate->second].end())
    lgGate = (findVal->second);
...
string LibraryParse::getGateLogicType(string gateName)
{
  ...
  if (findGate == wxGetApp().gateNameToLibrary.end())
    return "";
  if (gates[findGate->second].find(gateName) == gates[findGate->second].end())
    return "";
