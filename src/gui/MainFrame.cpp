...
{
  cout << "Entered mainframe constructor" << endl;
  // set the frame icon
  ...
  if (wxGetApp().appSettings.lastDir == "")
    ...
    lastDirectory = wxGetApp().appSettings.lastDir; // added cast KAS
  cout << "Last directory=" << lastDirectory << endl;

  //////////////////////////////////////////////////////////////////////////
  // create a menu bar
  //////////////////////////////////////////////////////////////////////////
  cout << "Creating menu bar" << endl;
  ...
  cout << "Attached menus to menu bar" << endl;
  ...
  cout << "Set checkmarks on settings menu" << endl;

  // ... and attach this menu bar to the frame
  SetMenuBar(menuBar);

  cout << "Attached menu bar to frame" << endl;

  //////////////////////////////////////////////////////////////////////////
  // parse a gate library
  //////////////////////////////////////////////////////////////////////////
  cout << "Parsing a gate library" << endl;
#ifndef _PRODUCTION_
  ...
  cout << "Set lib parser of app" << endl;

  //////////////////////////////////////////////////////////////////////////
  // create a toolbar
  //////////////////////////////////////////////////////////////////////////
  cout << "Creating toolbar" << endl;
  ...
  cout << "Will get bitmaps" << endl;

  for (int i = 0; i < 15; i++)
  {
    bitmaps[i] = wxGetApp().pathToExe + "res/bitmaps/" + bitmaps[i] + ".bmp";
    cout << "Getting bitmap " << bitmaps[i] << endl;
    wxFileInputStream in(bitmaps[i]);
    bmp[i] = new wxBitmap(wxImage(in, wxBITMAP_TYPE_BMP));
    cout << "Got bitmap " << bitmaps[i] << endl;
  }

  cout << "Got all bitmaps" << endl;

  cout << "Configuring toolbar bitmaps" << endl;
  ...
  cout << "Configured toolbar bitmaps" << endl;
  ...
  cout << "Deleted bitmaps" << endl;
  cout << "Creating status bar" << endl;

  CreateStatusBar(2);
  SetStatusText("");
  cout << "Created status bar" << endl;

  cout << "Creating sizers" << endl;
  mainSizer = new wxBoxSizer(wxHORIZONTAL);
  wxBoxSizer *leftPaneSizer = new wxBoxSizer(wxVERTICAL);
  wxSize sz = this->GetClientSize();
  cout << "Created sizers" << endl;

  // now a gate palette for the library
  cout << "Creating a gate pallete for library" << endl;
  ...
  cout << "Created gate pallete for library" << endl;
  ...
  cout << "Got command processors" << endl;

  cout << "Creating new canvas book" << endl;
  ...
  cout << "Added new canvas book" << endl;

  //add 1 tab: Left loop to allow for different default
  cout << "Adding default tab" << endl;
  for (int i = 0; i < 1; i++)
  {
  ...
  cout << "Created default tab" << endl;

  cout << "Setting current canveses" << endl;
  ...
  cout << "Focused on current canvas" << endl;
...
void MainFrame::loadCircuitFile(string fileName)
{
  ...
  while (!(wxGetApp().dGUItoLOGIC.empty()))
    wxGetApp().dGUItoLOGIC.pop_front();
  while (!(wxGetApp().dLOGICtoGUI.empty()))
...
void MainFrame::OnIdle(wxTimerEvent &event)
{
  ...
  while (wxGetApp().mexMessages.TryLock() == wxMUTEX_BUSY)
    wxYield();
...
void MainFrame::OnNotebookPage(wxAuiNotebookEvent &event)
{
  ...
  if (currentCanvas == NULL || canvases[canvasID] == currentCanvas)
    return;
  //**********************************
