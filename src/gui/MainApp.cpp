...
void MainApp::loadSettings()
{
  ...
  else
  {
    ...
    if (appSettings.refreshRate <= 0)
      appSettings.refreshRate = 60; // Check for valid refresh rate (0 or less is bad)
    // LastDirectory
    getline(iniFile, line, '\n');
    pos = line.find('=', 0);
    ...
    if (appSettings.wireConnRadius <= 0 || appSettings.wireConnRadius > 0.3)
      appSettings.wireConnRadius = 0.18f;
