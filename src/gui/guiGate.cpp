...
void guiGate::draw(bool color)
{
  ...
  if (selected && color)
  {
    cout << "Gate is selected and color is enabled" << endl;
  ...
  for (unsigned int i = 0; i < vertices.size(); i++)
  {
    ...
    cout << "Created vertex at " << vertices[i].x << ", " << vertices[i].y << endl;
  }
  glEnd();
...
{
  if (hotspots.find(connection) != hotspots.end())
    return; // error: hotspot already exists
  ...
  while (pParams != lparams.end())
  {
    ...
    for (unsigned int i = 0; i < lg.dlgParams.size() && !found; i++)
    {
      ...
      if ((lg.dlgParams[i].type == "FILE_IN" || lg.dlgParams[i].type == "FILE_OUT") &&
          lg.dlgParams[i].name == pParams->first)
        found = true;
    }
    if (found)
    {
...
{
  if (wxGetApp().libraries[libName][libGateName].dlgParams.size() == 0)
    return;
...
void guiGateTOGGLE::draw(bool color)
{
  ...
  //Inner Square
  if (color)
    glRectd(renderInfo_clickBox.begin.x, renderInfo_clickBox.begin.y,
...
void guiGateKEYPAD::draw(bool color)
{
  ...
  //Inner Square
  if (color)
    glRectd(renderInfo_valueBox.begin.x, renderInfo_valueBox.begin.y,
...
void guiGateKEYPAD::setLogicParam(string paramName, string value)
{
  if (paramName == "OUTPUT_NUM" || paramName == "OUTPUT_BITS")
  {
    ...
    iss.clear();
    iss.str(paramName == "OUTPUT_BITS" ? value : lparams["OUTPUT_BITS"]);
    iss >> intVal;
    ...
    if (gparams.find(ossParamName.str()) == gparams.end())
    {
      ...
      {
        if ((gparamWalk->first).substr(0, 11) != "KEYPAD_BOX_")
        {
          gparamWalk++;
          continue;
        }
...
void guiGateREGISTER::setLogicParam(string paramName, string value)
{
  ...
  if (paramName == "INPUT_BITS")
  {
    ...
    if (renderInfo_numDigitsToShow == 0)
      renderInfo_numDigitsToShow = 1;
    if (getLogicParam("CURRENT_VALUE") != "")
    {
      iss.clear();
      iss.str(getLogicParam("CURRENT_VALUE"));
      iss >> intVal;
...
void guiGateLED::draw(bool color)
{
  ...
  //Inner Square
  if (color)
    glRectd(renderInfo_ledBox.begin.x, renderInfo_ledBox.begin.y,
...
void guiLabel::calcBBox(void)
{
  ...
  theText.getPosition(currentX, currentY);
