...
gateImage::gateImage(string gateName, wxWindow *parent, wxWindowID id,
                     const wxPoint &pos,
                     const wxSize &size,
                     long style, const wxString &name) : wxWindow(parent, id, pos, size, style | wxFULL_REPAINT_ON_RESIZE, name)
{
  m_init = false;
  inImage = false;
...
void gateImage::setViewport()
{
  ...
  //Don't need to multiply by content scale since it would just cancel
